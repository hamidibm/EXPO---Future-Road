<?php 
    ...
    if(isset($_POST["registrasi"])){
        ...
        $duplikat = "SELECT email_pembeli FROM pembeli WHERE email_pembeli = '$email' ";
        $cek = mysqli_query($conn, $duplikat);
        $count = mysqli_num_rows($cek);

        $duplikat2 = "SELECT email_penjual FROM penjual WHERE email_penjual = '$email' ";
        ...
        if($count > 0){
            ...
            echo "<script>
                    alert('Email ini sudah terdaftar sebagai Pemliki UKM!');
                  </script>";
        }

        else{
            if(registrasiKonsumen($_POST) > 0){
                echo "<script>
...
<html>
    ...
    <body style="background-color:rgb(14, 124, 0);">
        <table border="0" style="background-color: gold; border: 10px solid rgb(0, 0, 0)" class="regisForm">
            <tr>
                <td>
                    <h3 style="color:black; text-align: center;">Registrasi UKMZone</h3>
                    <br>
                    <form method="POST" style="margin-right: auto; margin-left: auto">
                        ...
                        <input class="inputbox" style="color:black; background-color:white ;border-radius: 10px; border: solid black" type="text" name="alamat" placeholder="Masukkan alamat" required>
						<br>
                        <h4 style="color:black">No. Telpon</h4>
